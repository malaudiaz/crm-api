...
@contact_route.get("/contacts", response_model=ResultObject, summary="Obtener lista de Contactos")
def get_contacts(
    page: int = 1, 
    per_page: int = 6, 
    criteria_key: str = "",
...
@contact_route.get("/contacts/{id}", response_model=ResultObject, summary="Obtener un Contacto por su ID")
def get_contact_by_id(id: str, db: Session = Depends(get_db)):
    return get_one_contact(contact_id=id, db=db)

@contact_route.get("/contacts/partner/{partner_id}", response_model=ResultObject, summary="Obtener listado de Contactos de un Cliente")
def get_contacts(
    page: int = 1, 
    per_page: int = 6, 
    partner_id: str = "", 
...
@contact_route.post("/contacts", response_model=ResultObject, summary="Crear un Contacto")
def create_contact(contact: ContactBase, db: Session = Depends(get_db)):
    return new(contact=contact, db=db)

@contact_route.delete("/contacts/{id}", response_model=ResultObject, summary="Desactivar un Contacto por su ID")
...
@contact_route.put("/contacts/{id}", response_model=ResultObject, summary="Actualizar un Contacto por su ID")
